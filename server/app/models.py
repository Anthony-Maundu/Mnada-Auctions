...
class Item(db.Model):
    ...
    image_id = db.Column(db.Integer, db.ForeignKey('images.image_id'), nullable=True)  # Nullable for items without images
...
class Report(db.Model):
    ...
    details = db.Column(db.Text, nullable=True)  # Added for detailed descriptions
    status = db.Column(db.String(20), default="open")  # Status: open/closed
...
class Notification(db.Model):
    ...
    is_read = db.Column(db.Boolean, default=False)  # Added to track read status
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), nullable=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
